...
for line in open(f'{lang}-en-subword.dic.all'):
...
for index, line in enumerate(open('opus-mt-de-en/vocab.txt')):
    line = line.strip()
    dic_en_de[line] = index

dic_my = {}
for index, line in enumerate(open(f'{lang}-tokenizer/vocab.txt')):
    line = line.strip()
    dic_my[line] = index
...
for x in dic:
    ...
    try:
        my_id = dic_my[my]
        for e in en:
            dic_avg[my_id].append(dic_en_de[e])
    except:
        continue
